...
Если в репозитории есть `assets/start.gif`, `/start` отправит ее как стартовую анимацию. При ошибке отправки бот автоматически вернется к обычному текстовому сообщению.
...
Для offline-контроллеров бот может показывать локальный HTTP-доступ. IP берется из заметки контроллера в Wirenboard.cloud:

```json
{"label": "IP", "value": "192.168.1.1/16"}
```

Маска допускается, но для ссылки используется только адрес:

```text
http://192.168.1.1/
```

При старте сервиса бот проверяет все контроллеры, доступные WB-токену. Если заметки `IP` нет, он пробует создать:

```json
{"label": "IP", "value": "CHANGE_ME"}
```

После этого значение можно исправить в веб-интерфейсе Wirenboard.cloud. Админ может повторить проверку из Telegram: `/start` -> `Настройки/админские кнопки` или кнопка `Проверить IP-заметки` в админском меню.

Перед переходом на локальный адрес или кнопкой `Пинг` в Mini App нужно включить VPN до объекта. Проверка локального доступа выполняется с устройства пользователя, а не с VPS.

## Несколько организаций

WB-токен определяет область видимости. Если аккаунт Wirenboard.cloud имеет доступ к нескольким организациям, бот увидит контроллеры из всех этих организаций и будет создавать/проверять заметки `IP` для всех доступных контроллеров.

Telegram approved users сейчас получают общий список объектов, доступных WB-токену. Если нужно разделить пользователей по организациям, нужно включить отдельный слой прав по `organization.id`/`organization.name`.

## История, настройки и пользователи

Mini App содержит:

- `Статус` - текущие состояния;
...
Админы могут добавлять и удалять пользователей в Mini App по Telegram ID или известному `@username`. Username считается известным, если пользователь уже писал боту или открывал Mini App. Для нового пользователя, который еще не взаимодействовал с ботом, используйте Telegram ID.

Если неразрешенный пользователь откроет Mini App, он увидит экран `Доступ не выдан` и сможет отправить заявку. Админам придет Telegram-сообщение с ID/username и кнопкой `Разрешить пользователя`.
